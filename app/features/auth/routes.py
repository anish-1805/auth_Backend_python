...
from fastapi import APIRouter, Depends, HTTPException, status, Response, BackgroundTasks, Cookie, Request
...
async def verify_signup_otp(
    ...
    if not user.signupOTP:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"success": False, "message": "No OTP found. Please request a new one."},
        )
    ...
    otp_verification = verify_otp(
        body.otp,
        user.signupOTP["otp"],
        user.signupOTP["expiryTime"],
        user.signupOTP["isUsed"],
    )
    ...
    token = create_access_token({"userId": verified_user.id, "email": verified_user.email})
...
async def get_current_user_info(
    ...
    return AuthResponse(success=True, message="User retrieved successfully", user=current_user)
...
async def refresh_token(
    ...
    # Generate new token
    new_token = create_access_token({"userId": current_user.id, "email": current_user.email})
...
async def change_password(
    ...
    if verify_password(body.newPassword, user.password):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={"success": False, "message": "New password must be different from current password"},
        )
...
async def verify_password_reset_otp(
    ...
    otp_verification = verify_otp(
        ...
        user.passwordResetOTP["expiryTime"],
        user.passwordResetOTP["isUsed"],
    )
...
async def get_all_users(
    ...
    """
    users, total = await repository.get_all_users(page, limit, search, sortBy, sortOrder)
...
async def get_socket_token(
    ...
    """
    if jwt:
        return AuthResponse(success=True, data={"token": jwt})
    else:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"success": False, "error": "No token available"},
        )
...
async def google_callback(
    ...
    try:
        ...
        logger.info(f"✅ Google login successful for {email}, returning HTML with cookie")
